...
for some polynomial function over $\delta$.

\end{proof}

Once we know that such covering exists we can look for it on a rather difficult task. It in fact does not matter because we only have to run the search the first time we will run the algorithm because we can store it an used when necessary.\\
