...
We can incorporate clause learning to DPLL easily:
 \begin{algorithm}
  \caption{DPLL}\label{dpll}  \begin{algorithmic}[1]
   ...
   \Procedure{\texttt{propagate}}{$G$}
   \State $G_0 \gets G$
    \While{$G_0$ contains a unit clause or a pure literal$\{ l\}$} 
    \If{$G_0$ contains a unit clause $\{ l\}$} $G_0 = G_0\{l=1\}$)
    \EndIf
    \If{$G_0$ contains a pure literal $l$} $G_0 = G_0\{l=1\}$)
    \EndIf
\EndWhile
\State \Return $G_0$
\EndProcedure
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \EndIf
    \State
    \If{$F$ contains a unit clause $\{l\}$} \Return $MS(F\{l\to 1\})$
    \EndIf
