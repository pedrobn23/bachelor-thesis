...
These problems are among the most representatives of graphs theory, so we believe of great interest developing a library to solve then.
...
We now introduce one of the most active problems in graph theory. Some interesting problems such as the Hadwiger-Nelson problem. This area has an active scene. In this text we use the Lovasz Local Lemma, which was originally developed in order to solve an hypergraph (a generalization of graph) coloring problem.\\

We defined what a coloring is in Definition \ref{def:coloring}. We now change our focus and, instead for trying to find a stable coloring, we now look for \emph{proper coloring}.

\begin{definition}
  Let $G=(V,E)$ be a graph and $\pi$ be a coloring of $G$. We say that $\pi$ is a \emph{proper coloring} of $G$ if not two adjacent vertices belong to the same color.
\end{definition}
