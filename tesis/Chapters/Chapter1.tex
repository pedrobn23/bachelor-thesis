...
\end{definition}
	...
	A formula in \emph{CNF} could be seen as a collection of clauses. The associated problem with \emph{CNF} is straightforward on $O(n)$. The problem that we will investigate is whether a arbitrary formula $F$ have a \emph{SATe properties follows from the equivalent properties on the constants.
\end{proof}
