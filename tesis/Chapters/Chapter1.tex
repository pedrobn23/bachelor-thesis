...
TODO: las acciones de un grupo sirven para generalizar la teoría de grupos a las formulas, esto habría o que hacerlo mucho mejor o que quitarlo directamente. 
...
A little example of the prodedure may be of some help:
\begin{figure}[H]
  \centering
  ...
  \begin{tikzpicture}[scale = 0.5,node distance=3cm,on grid,auto,thick] 
    ...
    \node[state](q_2) [below right=of q_1,size = 1.5] {2};
    \node[state](q_3) [below left=of q_1] {3};
...
\end{figure}
 ...
 We now have a partition with only one set with all nodes. We will make a refinement by selecting the node with diferentent adjacency. Now we have a new coloring.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale = 0.5,node distance=3cm,on grid,auto,thick] 
    ...
    \node[state] (q_1) [below=of q_0] {$1$}; 
    \node[state](q_2) [below right=of q_1,size = 1.5, draw=blue!50,very thick,fill=blue!25] {2};
    \node[state](q_3) [draw=mycolor!50, fill=mycolor!25 , below left=of q_1] {3};
...
\item The second induces the symmetry of swaping 0 and 2.
\end{itemize}

\end{example}

Now that we have the right tool we can deepen in our study. In order to discober symmetries between in a formula $F$  we will apply this method to it associated symmetry graph $S_F$.
...
\subsection{Autraks assignments}
\label{sec:autark}
