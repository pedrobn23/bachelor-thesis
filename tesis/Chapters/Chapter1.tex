...
\begin{definition}[\cite{schoning2013satisfiability}]
  ...
  \item $C=\{C_1,...,C_m\}$ is the set of constraints over the variables. For our intentions, this constraints must be written as:
    \begin{itemize}
  ...
  such that every variable $x_i$ is mapped to a value on its associated domain $D_i$ and every constraint is satisfied. Such map will be called an \emph{assignment}, and if this map satisfy all constraints it will \emph{satisfy}.
\end{definition}
...
\begin{proposition} We could reduce FSAT to the Autark-Finding problem.
