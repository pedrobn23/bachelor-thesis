...
We continue to prove an interesting lemma on the theoretical analysis of satisfiability problem: the Lovász Local Lema (LLL). This lemma was first proven on 1972 by Erdos and Lovasz while they were studying 3-coloration of hypergraphs. Then it was Moser which understood the relationship between  this result an constraint satisfation problem. The SAT could be regard as the simplest of these problems. \\

%Incluir teorema pagina 20? 

 
This section is going to be based on the works of Moser, Tardos, Lovasz and Erdos as a result. As it will be shown LLL is applicable to set sufficent condition for satisfiability.  We will explain the lemma for theoretical purposes and prove the most general version, and give a constructive algorithm to solve a less general statement of the problem. The principal source of bibliography for the whole section would be Moser Phd. tesis. \\ 
%[INCLUIR LINK A LA BIBLIOGRAFIA]


The main contribution of Moser's works to this problem is finding an efficient algorithm to find what assigment satifies the formula, should happen that $F$ is proved satifiable by the previous theorem. Previously only probabilistics approaches had been succesful.\\


The probabilistic method is a useful method to prove the existence of objetcs with an especific property. The philosophy beneath this type of demostration is the following: in order to prove the existence of an object we do not need to give the said object, instead, we could just consider a random object in the space that we consider an prove that the probability is strictly positive. Then we can deduce that an object with that property exists (if it did not probability would be 0). It is not necesary to provide the exact value, bouning it by a constant greater that 0 would be enough. \\

This tecnique was pioneered by Paul Erdos. The LLL takes part because is an useful tool to prove lower bounds for probabilities, allowing us to provide the result.\\

This section will follow this order:
\begin{itemize}
	\item Present the notation and general expresion for the LLL.
	\item Use the result to prove an interesting property on satisfability un CNF.
...
We will work here with a very especific type of formulas. Let us call a formula $F$ is in $k$-CNF
...
\subsection{Statement of the Lovasz Local Lemma}
...
By considering the random experiment of drawing an assignment uniformuly, with the event corresponding to violating the different clauses we could refomulate this result. The weight of each clause is the probability of violating each clause. Therefore, we can state a SAT-focused result.
...
\end{corollary}
\begin{proof}
	To prove the result it would only be necesary to show  that $ \Gamma^*$ is the lopsidependency graph for this experiment. Given $C \in F$ and $\mathcal{D}\subset F\backslash \Gamma_{G_F^*}(D)\ $(i.e. no $D \in  \mathcal{D}$ conflict with $C$). We want to check the probability of a random assignment falisifying $C$ given that it satisfies all of the clauses in $\mathcal{D}$, and prove that it is at most $2^{-|C|}$. \\ 
	
Let $\alpha$ be an assigment such that it satisfies $\mathcal{D}$ and violates $C$. We could generate new assigment from $\alpha$ changing any value on $Var(C)$, and they still will satisfy $\mathcal{D}$ (as there are no conflict) so the probability is still at most $2^{-k}$. 
...
In order to get a result easier to check. If $k\le 2$ the $k$-SAT problem is  polinomically solvable so we will not be interested on such kormulas.

\begin{corollary}
	Let $F$ be a $k$-CNF with $k>2$ formula such that $\ \forall C \in F$ and $\ |\Gamma_F(C)|\le 2^k/e-1$ then $F$ is satisfable.
\end{corollary}
\begin{proof}
...
We explain the way Erdös, Lovász and Spencer originally proved the Lemma. This material is from [EL75] and [Spe77]. The write-up presented here will resemble the one done by [Moser], already apeared similarly in [MW11].\\



Thorough the proof we will use repeteadly the definition of conditional probability, i.e. for anu events $\{E_i\}_{i\in 1,...,r}$,

\[
P\left ( \bigcap_{i=1}^r E_1\right ) = \prod_{i=1}^rP \left( E_i \Big | \bigcap_{j=1}^{i-1} E_j\right)\\
\]

Further on this subsection we will consdier  $\Omega$ to be a probability space and $\mathcal{A} = \{A_1,...,A_m\}$ to be arbitrary events in this space, $G$ to be the lopsidependency graph, and $\mu: \mathcal{A} \to (0,1)$ with such that the conditions of the theorem are satisfied. We first prove an auxiliary lemma.
...
The proof is by induction on the size of $|\mathcal{H}|$. The case $H=\emptyset$ follows from the hipotesis easily:

$$ 
	P\left ( A \Big| \bigcap_{B\in \mathcal{H}} \overline{B}\right ) =  P(A) \le^{1.}   \mu(A) \prod_{B\in\Gamma^*_G(A)} (1-\mu(B)) \le^{2.} \mu(A) $$

Where 1. uses the hypotesis and 2. uses that $0 < \mu(B) < 1$. Now we supose that $|\mathcal{H}|=n$ and that the claim is true for all $\mathcal{H}'$ such that $|\mathcal{H}'|<n$. We distinguish two cases. The induction hipotesis will not be neccesary for the first of them
\begin{itemize}
...
Where the second inequiality is given by the definition of lopsidependency graph. On the other hand, for the denominator, we can define $\mathcal{H}_i := \{H_i,...,H_k\} \cup \mathcal{H}_0$.
\begin{align*}
...
Where in 3. the induction hypotesis is used, and in 4. is considering  that $H_i \in \Gamma_G^*(A)$
...
\]
Where the last inequality uses the hypotesis on $\mu$.
\end{itemize}
\end{proof}
...
Moser proves that it exists an algorithm such that it give an assigment satisfiying the SAT formula, should it happend that the formula satisfies \ref{LLLS} conditions. This is no a big deal, as a bactrack would be also capable of providing the solution, given that we know its existance. What is not trivial is that it would run in $O(|F|)$
\begin{algorithm}
